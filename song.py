...
class Song(Widget):
    def __init__(self, filepath, structure, selection, bar0_beat=4, gain=1, speed=1, **kwargs):
    ...
    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            ...
            self.down_pos_x = touch.pos[0]
            self.cum_mvt = 0

            return True

        else:
            return super(Song, self).on_touch_down(touch)
    ...
    def on_touch_up(self, touch):
        if touch.grab_current is self:
            ...
            if self.cum_mvt != 0:
                mvt = int(self.cum_mvt - (self.cum_mvt % (HORIZ_SCALE * 4)))
                beats_mvt = int(mvt / HORIZ_SCALE)

                self.pos[0] += mvt
                self.translate.x += mvt
    ...
    def on_touch_move(self, touch):
        if self.collide_point(*touch.pos):
            mov_x = touch.pos[0] - self.down_pos_x
            self.down_pos_x = touch.pos[0]

            # self.pos[0] += mov_x
            self.cum_mvt += mov_x
            # self.waveform_trans.x = self.pos[0]

            if int(self.cum_mvt / (HORIZ_SCALE * 4)) != 0:
                mvt = int(self.cum_mvt - (self.cum_mvt % (HORIZ_SCALE * 4)))
                beats_mvt = int(mvt / HORIZ_SCALE)
            ...
            return super(Song, self).on_touch_move(touch)
